...
"""
🏓 多任務分類模型 (重構版 v2.1) - 主執行檔
---------------------------------------------------------------------
🌟 v2.1 更新：
- 從 data_processing 匯入新的特徵工程流程。
- 新增 One-Hot Encoding 後的欄位對齊步驟。
"""
...
def apply_feature_selection(split_data, y_all, X_test, K_FEATURES):
    ...
    print(f"🔥 actionId Top 5: {top_features_action[:20]}")
    ...
    print(f"🔥 pointId Top 5: {top_features_point[:20]}")
    ...
    print(f"🔥 serverGetPoint Top 5: {top_features_server[:20]}")
...
def train_xgb_with_search(X_train, X_valid, y_train, y_valid, num_class, top_features, objective="multi:softmax", n_iter=25):
    ...
    if xgb.__version__ >= "2.0.0":
        fit_params["sample_weight_eval_set"] = [compute_sample_weight(class_weight='balanced', y=y_valid)]
    ...
    rand_search.fit(X_search, y_search, sample_weight=search_weights, **fit_params)
...
def main():
    ...
    if missing_in_test:
        for c in missing_in_test:
            # 只需處理 get_dummies 產生的 type_ 前綴欄位
    ...
    pred_point_test = pointid_model.predict(X_test[top_features_point])
